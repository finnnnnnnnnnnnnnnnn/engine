...
fn main() {
    ...
    let mut scope = Box::new(Arc::new(Scope::new()));

    for def in parsed {
        match def {
            D::Interface(defintion) => {
                let interface_name = defintion.identifier.0;
                let interface_struct = scope.as_mut().new_struct(interface_name).derive("Debug");
                ...
                for member in interface_members {
                    match member {
                        IM::Operation(operation) => {
                            ...
                            for arg in args{
                                match_argument(operation_fn, arg, Some(Arc::clone(&scope)));
                            }
                        },
                        IM::Constructor(constructor) => {
                            ...
                            for arg in args {
                                match_argument(construction_fn, arg, None);
                            }
...
fn match_argument(function: &mut codegen::Function, arg: Argument<'_>, scope: Option<Arc<Scope>>) {
    match arg {
        Argument::Single(single_arg) => {
            let single_arg_type = match_type(single_arg.type_.type_, scope);
...
fn match_return_type(return_type: ReturnType) -> codegen::Type {
    match return_type {
        ...
        ReturnType::Type(returned_type) => {
            match_type(returned_type, None)
        }
    }

}

fn match_type(returned_type: Type<'_>, scope: Option<Arc<Scope>>) -> codegen::Type {
    match returned_type {
